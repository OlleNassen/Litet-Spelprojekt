...
local totalFurthestSprites = 2

bgs[3] = Background:create()
bgs[3].texture = textureFunc("Resources/Sprites/Background/Pillar_diffuse.png")
bgs[3].sprite = newBackground(100, 1600, 0, bgs[3].texture)

bgs[4] = Background:create()
bgs[4].sprite = newBackground(100, 1600, 0, bgs[3].texture)
...
function updateBackground()
	...
	for k, v in pairs(bgs) do
		if k <= totalFurthestSprites then
			v:setPosition(pX * 0.01  + (400 * k), pY * -0.05 - 100, k)
		else
			index = k - totalFurthestSprites
			v:setPosition(pX * 0.1 + (index * 500), pY * -0.2, k)
		end
	end
end